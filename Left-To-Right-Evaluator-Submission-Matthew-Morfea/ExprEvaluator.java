...
public class ExprEvaluator
{
  ...
  private static void eval()
  {
      ...
      switch (op) //evaluate
      {
        ...
        case '/':
          val = opnd1/opnd2;
          break;
        case '^':   //Case in which Operator is a power symbol Ex: a ^ b = A to the power B
  ...
  private static boolean prec(char token, Stack1gen<Character> StackA)
  {
    ...
    if((token == '^'))
    {
      return true; // the '^' token has precedence over the other operators, so it will be pushed onto A
    }else{
        if((topOp == '^'))
      {
        return false; //operator at top of A is '^' and has precedence
      }else{
          if((token == '*') || (token == '/'))
          {
    ...
    do
    {
      ...
      else
      {
        ...
        else
        {
          if(c == '_') 
          {
             negflag += 1; //Counts the number of '_' (Unary Negative / Negative Symbol)
             //characters in this number being formed
          }
        }
      }
      ...
      count2--; //This count sets the number s to their proper decimal places if no decimals are present
    }while((p <= len) && (((d<='9')&&(d>='0'))||(d=='_')||(d=='.')));//check for a valid character
    //This if statement forms the number based on how many unary negative values are applied to this number
    if(negflag >= 1)
    {
        for(int i = 0; i < negflag; i++)
        {
            total = -(total); 
            count2++;
        }
    }
    //If decimal place is present we use equation 1 to calculate decimal number placement.
    //Otherwise we just calculate regular number placement based on count2.
    if(flag==1)
    {
     total = total / Math.pow(10.0,(count + 1)*1.0); //compute the value taking into account
                                                     //the number of decimal places
    }else{
        total = total / Math.pow(10.0,(count2 + 1)*1.0); //compute the value taking into account
        //the formation or termination of decimal places or negative symbols
  ...
  public double evaluator() 
  {
    ...
    {    
      ...
      if(token == '(')
      {   
              A.push(token); //always push a left parenthesis to delimit a subexpression
              p++;
      }
      ...
      //if the token is the leftmost  digit of a number or a decimal point on the right,
      //form the number as a double and push onto stack B
      //If the token is a negative symbol '_' then we form the number as a double and push this onto
      //stack B as well
      ...
      //If there is a space located in the expression we skip over the space to the next operand / operator
      //Space Handling
      if(token == ' ')
      {
          p++;
    ...
    }while(p <= len);//continue to scan from left to right
